...
export default function SharePostButton({ post }) {
  ...
  // URL du post — pointe vers le site PuzzleWorld (c'est ça qui fait la pub)
  const postUrl = `${SITE_URL}/social?post=${post.id}`;

  // Texte de partage : contenu du post tronqué + signature PuzzleWorld
  ...
  const handleNativeShare = async () => {
    ...
    // Essaye d'inclure l'image si disponible (Android/iOS la proposent à Instagram etc.)
    ...
    try {
      ...
      if (err.name !== 'AbortError') setShowPanel(true); // fallback desktop si erreur
    }
  };
  ...
  // ── Bouton principal ───────────────────────────────────────────────────────
  // Sur mobile avec Web Share API disponible → partage natif direct
  ...
  // Sur desktop → bouton qui ouvre le panel de partage
  return (
      ...
      {showPanel && (
        <>
          ...
          <div className="absolute bottom-8 left-0 z-50 bg-[#1a1a2e] border border-white/10 rounded-2xl p-4 shadow-xl shadow-black/50 w-64">
            ...
            {/* Aperçu du contenu */}
            {postContent && (
              <p className="text-white/40 text-xs mb-3 line-clamp-2 italic">
                "{postContent}"
              </p>
            )}
            ...
            <p className="text-white/20 text-[10px] text-center mt-3">
              🧩 Le lien inclut un aperçu PuzzleWorld
            </p>
          </div>
        </>
      )}
