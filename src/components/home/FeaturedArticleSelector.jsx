...
export default function FeaturedArticleSelector({ open, onClose, position, currentArticle, onUpdate }) {
  ...
  useEffect(() => { if (open) { loadCategories(); loadArticles(); } }, [open]);
  useEffect(() => { if (open) loadArticles(); }, [search, sortBy, filterCategory]);

  const loadCategories = async () => {
    try {
      const { data } = await supabase.from('blog_articles').select('category').eq('is_published', true);
      const uniqueCats = [...new Set((data || []).map(a => a.category).filter(Boolean))].sort();
  ...
  const loadArticles = async () => {
    setLoading(true);
    try {
      let query = supabase.from('blog_articles').select('*').eq('is_published', true).order('created_at', { ascending: false }).limit(100);
      ...
      if (sortBy === 'category') filtered.sort((a, b) => (a.category || '').localeCompare(b.category || ''));
  ...
  const selectArticle = async (article) => {
    ...
    try {
      if (currentArticle) {
        const { error } = await supabase.from('featured_articles').update({
  ...
  const removeArticle = async () => {
    ...
    setSaving(true);
    try {
      await supabase.from('featured_articles').delete().eq('id', currentArticle.id);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-[#0a0a2e] border-white/10 text-white max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="relative mb-3">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un article..." className="pl-10 bg-white/5 border-white/20 text-white" />
        </div>
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            <label className="text-xs text-white/60 block mb-1.5">Catégorie</label>
            <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="w-full bg-white/5 border border-white/10 text-white text-sm rounded px-3 py-2">
          ...
          <div>
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="w-full bg-white/5 border border-white/10 text-white text-sm rounded px-3 py-2">
        ...
        {currentArticle && (
          <div className="mb-3 flex items-center justify-between p-3 bg-orange-500/10 border border-orange-500/30 rounded-lg">
            <span className="text-orange-400 text-sm">Actuel : {currentArticle.article_title}</span>
        ...
        <div className="overflow-y-auto flex-1 space-y-2">
          {loading ? (
            ...
            <button key={article.id} onClick={() => selectArticle(article)} disabled={saving}
              ...
              {article.cover_image
                ...
                : <div className="w-14 h-14 bg-white/5 rounded-lg flex items-center justify-center flex-shrink-0"><BookOpen className="w-6 h-6 text-white/20" /></div>
              }
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium text-sm line-clamp-2">{article.title}</p>
                {article.category && <span className="text-orange-400 text-xs">{article.category}</span>}
              </div>
            </button>
          ))}
        </div>
